...
// ============================================================
// 2. DOM SELECTORS AN TOÀN (CÔ LẬP TUYỆT ĐỐI)
// ============================================================
const getLoanInput = () => 
...
// ============================================================
// 3. HÀM TÍNH TOÁN CHÍNH (ĐÃ FIX CHUẨN ĐÚNG THEO YÊU CẦU)
// ============================================================
function calc() {
    ...
    // Cấu hình hằng số lãi suất phẳng 1.19% và phí thu hộ 20k
    const MONTHLY_RATE = 1.19 / 100 / term 
    const COLLECTION_FEE = 20000; 

    let monthly_payment = 0;
    let total_payment = 0;

    if (loan > 0 && term > 0) {
        // Gốc hằng tháng = Khoản vay / Kỳ hạn (Kỳ hạn chỉ chia cho khoản vay)
        const tien_goc_hang_thang = loan / term;
        
        // Lãi hằng tháng = Khoản vay x Lãi suất phẳng
        const tien_lai_thang = (loan * MONTHLY_RATE) * term;
        
        // Số tiền hằng tháng = Tiền gốc + Tiền lãi + Phí thu hộ
        monthly_payment = tien_goc_hang_thang + tien_lai_thang + COLLECTION_FEE;
    }
    ...
    // 2. Tự động hiển thị Tổng số tiền cuối kỳ ngay bên dưới khung kết quả
    ...
    // 3. Đồng bộ vào form liên hệ ngầm
    ...
    if (rateLoanInput)   rateLoanInput.value   = "1.19%/tháng";
}
...
function initContactForm() {
    ...
    btn.addEventListener('click', async (e) => {
        ...
        if (typeof emailjs === 'undefined') {
            showToast('Hệ thống đang tải, vui lòng thử lại sau 2 giây!');
            return;
        }
        ...
        if (!name) { showError(nameEl, 'Vui lòng nhập Họ và tên'); return; }
        if (!phone || !/^0[3-9]\d{8}$/.test(phone.replace(/\s/g, ''))) { showError(phoneEl, 'Số điện thoại không hợp lệ'); return; }
        if (!vehicle) { showError(vehicleEl, 'Vui lòng nhập loại xe'); return; }
        ...
        try {
            ...
            if (result && (result.status === 200 || result.text === 'OK')) {
                ...
                throw new Error('Lỗi gửi: ' + result.status);
            }
        } catch (err) {
            console.error('EmailJS error:', err);
            btn.innerHTML = origHTML;
            btn.disabled  = false;
            showToast('Gửi thất bại. Vui lòng gọi hotline!');
        }
    });
}

function showError(inputEl, msg) {
    if (!inputEl) { showToast(msg); return; }
...
function showToast(msg) {
    ...
    t.style.cssText = `position:fixed;bottom:24px;left:50%;transform:translateX(-50%);background:#EE0000;color:#fff;padding:12px 22px;border-radius:8px;font-size:13px;font-weight:600;z-index:9999;box-shadow:0 4px 16px rgba(0,0,0,.2);animation:hv-fadeup .25s ease;`;
    t.textContent = msg;
...
function showSuccess(container) {
    ...
    container.innerHTML = `
        <div style="display:flex;flex-direction:column;align-items:center;padding:48px 20px;gap:14px;text-align:center;">
            ...
            <p style="font-size:13px;color:#666;max-width:280px;line-height:1.75;margin:0">Chúng tôi đã nhận được thông tin và sẽ liên hệ lại sớm nhất.</p>
            <div style="margin-top:8px;padding:14px 20px;background:#f8f9fa;border:1px solid #ddd;border-radius:8px;font-size:13px;color:#333;">
                Hoặc gọi ngay:<br>
                <strong style="color:#EE0000;font-size:15px">0931 434 648</strong> — <strong style="color:#EE0000;font-size:15px">0901 264 949</strong>
            </div>
        </div>`;
}
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function renderCustomDropdown(keyword = '') {
        ...
        const filteredBikes = bikeModels.filter(bike => bike.toLowerCase().includes(keyword.toLowerCase().trim()));
    ...
    searchInput.addEventListener('focus', function() { renderCustomDropdown(this.value); dropdownList.classList.add('moto-search-active'); });
    searchInput.addEventListener('input', function() { renderCustomDropdown(this.value); dropdownList.classList.add('moto-search-active'); });
    document.addEventListener('click', function(e) { if (!searchInput.contains(e.target) && !dropdownList.contains(e.target)) { dropdownList.classList.remove('moto-search-active'); } });
});
