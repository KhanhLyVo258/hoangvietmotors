...
function initContactForm() {
    ...
    btn.addEventListener('click', async (e) => {
        ...
        const cfInputs = document.querySelectorAll('.step-card:nth-child(2) .cf-input');
        const name     = cfInputs[0]?.value.trim() || '';
        const phone    = cfInputs[1]?.value.trim() || '';
        const vehicle  = cfInputs[2]?.value.trim() || '';
        const email    = cfInputs[3]?.value.trim() || '';
        ...
        // Đọc giá trị khoản vay thực tế từ thanh kéo Slider để xuất dữ liệu gửi Mail công bằng
        const loanSliderEl = getLoanSlider();
        const termSliderEl = getTermSlider();
        const resultBoxEl  = getResultBox();
        ...
        // Kiểm tra hợp lệ dữ liệu nhập (Validate)
        // ── Quy trình kiểm tra lỗi nhập liệu (Validate) ──
if (!name) {
    showError(cfInputs[0], 'Vui lòng nhập Họ và tên'); return;
}
if (!phone || !/^0[3-9]\d{8}$/.test(phone.replace(/\s/g, ''))) {
    showError(cfInputs[1], 'Số điện thoại không hợp lệ (VD: 0901234567)'); return;
}
if (!vehicle) {
    showError(cfInputs[2], 'Vui lòng nhập loại xe'); return;
}
if (!province) {
    showError(document.getElementById('province-select'), 'Vui lòng chọn tỉnh/thành'); return;
}
